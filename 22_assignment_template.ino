...
// Framework setting
#define _DIST_TARGET 235 //[2967] 탁구공 위치까지의 거리를 255로 고정
...
#define _SERVO_SPEED 80  //[2959] 서보의 속도 설정
...
// PID parameters
#define _KP_servo_up 1 // [2957] 비례 제어 값. 서보 각 증가시. (아래쪽으로)
#define _KP_servo_down 2.3 // (위쪽으로)
...
////////////////////
  ...
  // PID control logic
    ...
    if (error_curr > 0) pterm = _KP_servo_up * error_curr;
    else pterm = _KP_servo_down * error_curr; 
    control = pterm;

  // duty_target = f(duty_neutral, control)
    duty_target = _DUTY_NEU + control;
    if (duty_target < _DUTY_REAL_MIN) duty_target = _DUTY_REAL_MIN;
    if (duty_target > _DUTY_REAL_MAX) duty_target = _DUTY_REAL_MAX;
...
float ir_distance(void){ // return value unit: mm
  ...
  // 주기적으로 크게 튀는값 제거
  if (former_reading==0) former_reading = val;
  else if (val < 220 && (former_reading - val)/former_reading > 0.1){
    val = former_reading;
    former_reading = val * ((former_reading - val)/val+1);
  }
  else if (val >= 220 && (former_reading - val)/former_reading > 0.15){
    val = former_reading;
    former_reading = val * ((former_reading - val)/val+1);
  }
  else former_reading = val;
  
  // calibrate
  if (val<94) return val*0.825;
  else if (val <148) {
    a = 94;
    b = 148;
    a_ = 80;
